<template>
  <tr v-for="(payment, index) in payments" :key="index" @click="showPaymentInfo(payment)"
      ...
      <div class="flex items-center">
        <div
          ...
          <img class="object-cover w-full h-full rounded-full"
               :src="URL + '/member/image/' + payment.member.image" alt="#"/>
        </div>
        <div>
    ...
    <td class="hidden px-4 py-3 whitespace-nowrap md:table-cell">{{ paymentTypeTranslate(payment.payment.paymentType) }}</td>
    <td class="hidden px-4 py-3 whitespace-nowrap md:table-cell">
