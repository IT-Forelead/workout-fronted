<template>
  <div>
    <div class="px-2 md:px-5">
      <div class="z-0 flex items-center justify-between p-3 mb-5 sticky-top top-16 bg-slate-100 dark:bg-gray-900 md:top-20">
        ...
        <div class="flex items-center md:hidden">
          ...
          <button @click="openAddMemberModal()" class="mx-1 rounded-lg bg-blue-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 sm:w-auto">
        ...
        <div class="relative bg-white rounded-lg shadow-lg dark:bg-gray-800">
            ...
            <div class="flex p-3 mb-3 border border-gray-300 rounded-lg justify-evenly dark:border-gray-600 md:grid md:grid-cols-3">
              <div class="flex items-center justify-between">
                ...
                <div v-if="registerStatus.inProgress" class="flex items-center justify-between">
                  <div class="flex items-center justify-center w-10 h-10 font-semibold text-blue-500 bg-white border-2 border-blue-500 rounded-full text-md dark:bg-gray-700">01</div>
              ...
              <div class="flex items-center justify-between">
                ...
                <div v-if="checkingStatus.default" class="flex items-center">
                  <div class="flex items-center justify-center w-10 h-10 font-semibold text-gray-500 bg-white border-2 border-gray-300 rounded-full text-md dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">02</div>
              ...
              <div v-if="congratulationStatus.inProgress" class="flex items-center">
                <div class="flex items-center justify-center w-10 h-10 font-semibold text-blue-500 bg-white border-2 border-blue-500 rounded-full text-md dark:bg-gray-700">03</div>
          ...
          <form @submit.prevent="createMember()" method="post" enctype="multipart/form-data">
            <div v-if="registerMemberProcess.registerMode" class="flex flex-col p-5 md:mb-5">
              <label v-if="!member.image" for="dropzone-file" class="relative flex items-center justify-center w-24 h-24 max-w-lg p-6 mx-auto text-center border-2 border-blue-400 border-dashed rounded-full cursor-pointer bg-slate-100 dark:bg-gray-700">
                <PictureIcon />
                <input id="dropzone-file" type="file" class="hidden" name="image" @change="getImage" />
                <span class="absolute mx-auto mt-3 text-lg font-semibold tracking-wide text-blue-500 -bottom-10 whitespace-nowrap"> Fotosuratni yuklash</span>
              </label>
              <label v-else for="dropzone-file" class="relative flex items-center justify-center w-24 h-24 max-w-lg mx-auto text-center border-2 rounded-full cursor-pointer">
                <img :src="selectedImage" class="object-cover w-24 h-24 rounded-full" alt="#" />
                <input id="dropzone-file" type="file" class="hidden" name="image" @change="getImage" />
                <span class="absolute mx-auto mt-3 text-lg font-semibold tracking-wide text-blue-500 -bottom-10 whitespace-nowrap"> Boshqa fotosuratni yuklash</span>
              </label>
            </div>
            <!-- Step 1 -->
            <div v-if="registerMemberProcess.registerMode" class="grid p-5 md:mb-6 md:gap-6 lg:grid-cols-2">
              <div class="p-3">
                ...
                <input type="text" v-model="member.firstname" id="firstname" name="firstname" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500" placeholder="Ism kiriting" required />
              </div>
              <div class="p-3">
              ...
              <div class="p-3">
                ...
                <input name="phone" v-model="member.phone" v-mask="'+###(##) ###-##-##'" id="phone" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500" placeholder="+998(90) 123-45-67" required />
              </div>
              <input name="code" v-model="confirmCode" type="hidden" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500" required />
            </div>
            <!-- Step 2 -->
            <div class="flex justify-center" v-if="registerMemberProcess.checkingMode">
              <div class="flex flex-col">
                ...
                <p class="px-3 text-xl text-center text-gray-600 dark:text-gray-300">
                  <strong class="text-black dark:text-gray-300">{{ member.phone }}</strong> telefon raqamiga tasdiqlash kodi SMS tarzida jo'natildi!
                </p>
                ...
                <div v-if="showResendSMS" class="flex justify-center my-3 text-lg text-red-500 hover:underline"><a href="#" @click="getMemberData()">SMS xabarnoma kelmadimi?</a></div>
            ...
            <div v-if="registerMemberProcess.checkingMode" class="flex items-center justify-end p-5 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
              <div>
                <button v-if="registerMemberProcess.checkingMode && !showCheckBtn && !lastProgressBtn" class="px-4 py-2 font-medium text-white bg-green-500 rounded shadow-lg outline-none disabled:cursor-not-allowed disabled:bg-gray-400/80 disabled:shadow-none" disabled>Tasdiqlash</button>
                <button v-if="showCheckBtn" type="submit" class="px-4 py-2 font-medium text-white transition-colors duration-200 bg-green-500 rounded shadow-lg outline-none hover:bg-green-600 focus:bg-green-500 focus:ring-green-500 focus:ring-offset-2 active:scale-95 active:shadow-none disabled:cursor-not-allowed disabled:bg-gray-400/80 disabled:shadow-none">Tasdiqlash</button>
                <button v-if="lastProgressBtn" class="flex items-center px-4 py-2 font-medium text-white transition-colors duration-200 bg-green-400 rounded shadow-lg outline-none hover:bg-green-600 focus:bg-green-500 focus:ring-green-500 focus:ring-offset-2 active:shadow-none disabled:cursor-not-allowed disabled:bg-green-400/80 disabled:shadow-none" disabled>
                  <SpinIcon />
                  Tasdiqlash
                </button>
              </div>
          ...
          <div v-if="!registerMemberProcess.checkingMode" class="flex items-center justify-end p-5 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
            <div>
              <button v-if="registerMemberProcess.registerMode" @click="clearFields()" class="px-4 py-2 mr-2 font-medium text-white transition-colors duration-200 bg-teal-500 border border-teal-500 rounded outline-none hover:bg-teal-400 hover:text-white focus:ring-teal-600 focus:ring-offset-2 active:scale-95 disabled:cursor-not-allowed disabled:bg-gray-400/80 disabled:shadow-none">Tozalash</button>
              <button v-if="registerMemberProcess.registerMode" @click="getMemberData()" class="px-4 py-2 font-medium text-white transition-colors duration-200 bg-indigo-500 rounded shadow-lg outline-none hover:bg-indigo-600 focus:bg-indigo-600 focus:ring-indigo-600 focus:ring-offset-2 active:scale-95 active:shadow-none disabled:cursor-not-allowed disabled:bg-gray-400/80 disabled:shadow-none">Keyingi qadam</button>
              <button v-if="registerMemberProcess.congratulationMode" @click="closeAddMemberModal()" class="px-4 py-2 font-medium text-white transition-colors duration-200 bg-indigo-500 rounded shadow-lg outline-none hover:bg-indigo-600 focus:bg-indigo-600 focus:ring-indigo-600 focus:ring-offset-2 active:scale-95 active:shadow-none disabled:cursor-not-allowed disabled:bg-gray-400/80 disabled:shadow-none">Yakunlash</button>
            </div>
          </div>
        </div>
      </div>
