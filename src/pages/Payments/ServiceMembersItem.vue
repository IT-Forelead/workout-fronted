<template>
  <tr v-for="(payment, index) in payments" :key="index" @click="showPaymentInfo(payment)"
    ...
    <td class="hidden px-4 py-3 whitespace-nowrap md:table-cell">
      <p>{{ formatDateTime(payment.serviceMembers.createdAt) }}</p>
      <p>{{ payment.serviceMembers.expireAt ? formatDateTime(payment.serviceMembers.expireAt) : '' }}</p>
    </td>
    <td class="hidden px-4 py-3 whitespace-nowrap md:table-cell">
    ...
    <td v-if="payment.trainerService" class="hidden px-4 py-3 whitespace-nowrap md:table-cell">
      <div>{{ payment.trainerService.trainerName + ' - ' + payment.trainerService.name }}</div>
      <div>
        {{ payment.trainerService.price.toLocaleString('en-US') }}
        <span class="text-xs">UZS</span>
      </div>
    </td>
    ...
    <td class="hidden px-4 py-3 whitespace-nowrap md:table-cell">
      <div>
        {{ payment.serviceMembers.totalPrice.toLocaleString('en-US') }}
        <span class="text-xs">UZS</span>
      </div>
      <div>
        {{ payment.serviceMembers.pricePaid.toLocaleString('en-US') }}
        <span class="text-xs">UZS</span>
      </div>
    </td>
    <td class="hidden px-4 py-3 whitespace-nowrap md:table-cell">
      <span class="p-1.5 px-3 text-sm rounded-full" :class="paymentStatusClass(payment.serviceMembers.paymentStatus)">
        {{ paymentStatusTranslate(payment.serviceMembers.paymentStatus) }}
      </span>
    </td>
  </tr>
...
import { phoneStyle, paymentStatusTranslate, paymentStatusClass, formatDateTime } from "../../../utils/utils.js";
